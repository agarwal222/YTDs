...
electron_1.ipcMain.handle("settings:select-download-path", async () => {
    ...
    return store.get("downloadPath", electron_1.app.getPath("downloads")); // Return current if cancelled
});
    ...
    try {
        ...
        if (!fs.existsSync(filePath)) {
            ...
            if (fs.existsSync(dirPath)) {
                electron_1.shell.openPath(dirPath); // Open containing directory instead
        ...
        electron_1.shell.showItemInFolder(path.normalize(filePath)); // Reveals the file in the folder
        return true;
    }
    catch (e) {
...
electron_1.ipcMain.handle("downloads:remove-item", async (_event, itemId) => {
    ...
    try {
        ...
        if (updatedHistory.length < history.length) {
            ...
            if (win)
                win.webContents.send("downloads:updated"); // Notify renderer
            return true;
        }
            ...
            if (code === 0 && jsonData) {
                try {
                    ...
                    if (!formats) {
                        ...
                        if (info.entries) {
                            throw new Error("Playlist detected, but format fetching for individual items not implemented here.");
                        }
                        else {
                            throw new Error("No video/format information found in yt-dlp output.");
                        }
                    }
...
electron_1.ipcMain.handle("yt:download", async (_event, options) => {
    ...
    try {
        ...
        if (existingItemIndex > -1) {
            ...
            updatedHistory = history.map((item, index) => index === existingItemIndex ? pendingItem : item);
        }
        else {
            // Add new item
    ...
    if (options.startTime || options.endTime) {
        args.push("--download-sections", `*${options.startTime || ""}-${options.endTime || ""}` // Format: *start-end
        );
    ...
    try {
        ...
        proc.on("close", (code) => {
            console.log(`[close ${videoId}] Code: ${code}, Final Path: ${finalPath || "(not detected)"}`);
...
function updateStoreItemFinal(videoId, finalData) {
    try {
        const history = store.get("downloadHistory", []);
        const updatedHistory = history.map((item) => item.id === videoId ? { ...item, ...finalData } : item);
